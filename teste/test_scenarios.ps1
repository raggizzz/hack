# Scripts de teste PowerShell para os 4 cenários do Agente Validador Sandbox CAIXA
# Execute: .\test_scenarios.ps1

$BaseURL = "http://localhost:8080"

Write-Host "=== TESTES DO AGENTE VALIDADOR SANDBOX CAIXA ===" -ForegroundColor Green
...
function Invoke-PostRequest {
    ...
    try {
        $response = Invoke-RestMethod -Uri "$BaseURL$Endpoint" -Method POST -Body $Data -ContentType "application/json"
...
Invoke-PostRequest "/score-api/score" $Cenario1 "CENÁRIO 1: Habitação - Push 3 dias"
...
Invoke-PostRequest "/score-api/score" $Cenario2 "CENÁRIO 2: Cartões - Open Finance"
...
Invoke-PostRequest "/score-api/score" $Cenario3 "CENÁRIO 3: Bem-estar - Clubes de Mangá"
...
Invoke-PostRequest "/score-api/score" $Cenario4 "CENÁRIO 4: LGPD Incompleto (deve bloquear)"

# TESTES DE BUSCA NA KB
Write-Host "--- TESTE: Busca na Base de Conhecimento ---" -ForegroundColor Yellow
Invoke-GetRequest "/kb/kb/search?q=LGPD&top_k=3" "Busca por LGPD"
Invoke-GetRequest "/kb/kb/search?q=scoring&top_k=2" "Busca por scoring"
Invoke-GetRequest "/kb/kb/search?q=metodologia&top_k=3" "Busca por metodologia"

# TESTE DE HEALTH CHECK
Invoke-GetRequest "/health" "Health Check"
Invoke-GetRequest "/" "Endpoint Raiz"

Write-Host "=== TESTES CONCLUÍDOS ===" -ForegroundColor Green
Write-Host "Verifique os resultados acima para validar:"
Write-Host "1. Cenário 1 (Habitação): Score ~70, Fase 2, TRL 6" -ForegroundColor Cyan
Write-Host "2. Cenário 2 (Open Finance): Score ~78, Fase 2+, TRL 7" -ForegroundColor Cyan
Write-Host "3. Cenário 3 (Bem-estar): Score ~72, Fase 2, TRL 6" -ForegroundColor Cyan
Write-Host "4. Cenário 4 (LGPD): Score baixo, bloqueio por LGPD" -ForegroundColor Cyan
Write-Host "5. KB Search: Resultados relevantes" -ForegroundColor Cyan
Write-Host "6. Health: Status healthy" -ForegroundColor Cyan